...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_wandb_resume(opt) and not check_comet_resume(opt) and not opt.evolve:
        ...
        if yaml_files:
            ...
            opt.data = yaml_files[-1]
        else:
            opt.data = ROOT / 'data/coco.yaml'
        ...
        now = time
        modelname = f"{now.localtime().tm_year}-{now.localtime().tm_mon}-{now.localtime().tm_mday}-{now.localtime().tm_hour}-{now.localtime().tm_min}-{now.localtime().tm_sec}"
        opt.save_dir = './save_model' + '/' + modelname
