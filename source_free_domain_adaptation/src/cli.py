...
class MyLightningCLI(LightningCLI):
    def add_arguments_to_parser(self, parser):
        ...
        parser.add_optimizer_args((Adam, SGD, AdamW), link_to='model.init_args.optimizer_init')
    ...
    def instantiate_trainer(self, **kwargs: Any) -> Trainer:
        ...
        version = 'test'
        wandb_logger = WandbLogger(project="FaceDA", name=log_name)
        tensorboard_logger = TensorBoardLogger(log_dir, log_name, version)
        csv_logger = CSVLogger(log_dir, log_name, version)

        # 3. define callback for Checkpoint, LR Scheduler
        save_dir = os.path.join(log_dir, log_name, version)
        ...
        model_checkpoint_callback = ModelCheckpoint(
            ...
            monitor='valid/data1_acc', mode='max', auto_insert_metric_name=False
        )
        ...
        logger = callback = []
        if subcommand == 'fit':
            logger = [wandb_logger, tensorboard_logger, csv_logger]
