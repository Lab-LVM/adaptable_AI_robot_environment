...
corruption_name = ['Gaussian Noise', 'Shot Noise', 'Impulse Noise', 'Defocus Blur',
                   'Forested Glass Blur', 'Motion Blur', 'Zoom Blur', 'Snow', 'Frost', 'Fog',
...
class my_dataset:
    root = '/home/hankyul/shared/hdd_ext/hdd4000/face/jpg/MFR1'
    img_path_list = glob.glob(os.path.join(root, 'Unmasked', '*', '*.jpg'))
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(img_path).resize((224, 224), 3)
        #         print(f"path: {self.img_path_list[idx]}")
        x = np.asarray(img)
        return img_path, x
...
if __name__ == '__main__':
    ...
    for s in tqdm(range(4,6)):
        for cor in tqdm(range(15)):
            cor_root = os.path.join(ds.root, 'corrupted', f'severity_{s}', str(cor))
            for i in tqdm(range(len(ds))):
                img_path, img = ds[i]
                img_cor = corrupt(img, severity=s, corruption_number=cor)
                folder_name, image_name = img_path.split('/')[-2:]

                img_path_cor_folder = os.path.join(cor_root, folder_name)
                Path(img_path_cor_folder).mkdir(parents=True, exist_ok=True)

                img_cor = Image.fromarray(img_cor)
                img_cor.save(os.path.join(img_path_cor_folder, image_name))